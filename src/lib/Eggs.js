...
#read -p "Adesso puoi avviare un qualsiasi pc dalla rete via pxe con questa distribuzione. Premi un tasto per continuare..."`;
  ...
  initramfs(){
    ...
    */
    ...
    execAndShow(`cp ${this._fsDir}/initrd.img-${this._kernelVer}  ${this._tftpRoot}/${this._distroName}`);
    console.log(`### file initramfs ###`)
  }

  tempInstallerFinalize(){
    execAndShow(`mkdir ${this._tftpRoot}/${this._distroName}`);
    //console.log(`### Copia di initrd.img-${this._kernelVer} ###`)
    //execAndShow(`mv ${this._fsDir}/tmp/initrd.img-${this._kernelVer}  ${this._tftpRoot}/${this._distroName}`);
    console.log(`### Copia di vmlinuz-${this._kernelVer} ###`)
    execAndShow(`cp /boot/vmlinuz-${this._kernelVer}  ${this._tftpRoot}/${this._distroName}`);
...
APPEND root=/dev/nfs initrd=${this._distroName}/initrd.img-${this._kernelVer} nfsroot=${this._netBootServer}:${this._fsDir} ip=dhcp rw 
IPAPPEND 3
TEXT HELP
...
ENDTEXT
  ...
  include common.cfg`;
    ...
    shell.exec(`ln /usr/lib/PXELINUX/lpxelinux.0  ${this._tftpRoot}/lpxelinux.0`);
    shell.exec(`ln src/assets/undionly.kpxe ${this._tftpRoot}/undionly.kpxe`);
    shell.exec(`ln src/assets/wallpaper.png ${this._tftpRoot}/wallpaper.png`);

    shell.exec(
      `ln /usr/lib/syslinux/modules/bios/vesamenu.c32 ${this._tftpRoot}/vesamenu.c32`
    );
    shell.exec(
      `ln /usr/lib/syslinux/modules/bios/ldlinux.c32 ${this._tftpRoot}/ldlinux.c32`
    );
    shell.exec(
      `ln /usr/lib/syslinux/modules/bios/libcom32.c32 ${this._tftpRoot}/libcom32.c32`
    );
    shell.exec(
      `ln /usr/lib/syslinux/modules/bios/libutil.c32 ${this._tftpRoot}/libutil.c32`
    );
