...
class Arises {
  ...
  partDrive(destinationdrive){
    ...
    utils.exec(`sleep 1`);
    utils.exec(`parted -s ${destinationdrive} unit mb print free | grep Free | awk '{print $3}' | cut -d "M" -f1`);

    // Crea partizione boot
    utils.exec(`parted --script ${destinationdrive} mkpart primary ext4 1 512`);
    utils.exec(`parted --script --align optimal  ${destinationdrive} set 1 boot on`);
    utils.exec(`sleep 1`);

  // Crea partizione lvm
  	utils.exec(`parted --script --align optimal ${destinationdrive} mkpart primary ext2 512 $HD_SIZE_MB`);
